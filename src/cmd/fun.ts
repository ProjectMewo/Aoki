...
export default new class Fun extends Command {
  ...
  // 8ball command
  async "8ball"(i: ChatInputCommandInteraction, query: string, util: Utilities): Promise<Message> {
  ...
  // affirmation command
  async affirmation(i: ChatInputCommandInteraction): Promise<Message> {
    return await this.fetchAndSend(i, "https://www.affirmations.dev", "affirmation");
  };

  // advice command
  async advice(i: ChatInputCommandInteraction): Promise<Message> {
    return await this.fetchAndSend(i, "https://api.adviceslip.com/advice", "slip.advice");
  };

  // fact command
  async fact(i: ChatInputCommandInteraction, _: string, util: Utilities): Promise<Message> {
  ...
  // today command
  async today(i: ChatInputCommandInteraction, _: string, util: Utilities): Promise<Message> {
  ...
  // ship command
  async ship(i: ChatInputCommandInteraction, _: string, util: Utilities): Promise<Message> {
  ...
  // fortune command
  async fortune(i: ChatInputCommandInteraction, _: string, util: Utilities): Promise<Message> {
    const cookies = await util.getStatic("fortune");
    const cookie = util.random(cookies);
    return await i.editReply({ content: cookie });
  };

  // truth command
  async truth(i: ChatInputCommandInteraction, _: string, util: Utilities): Promise<Message> {
    const questions = await util.getStatic("truth");
    const question = util.random(questions);
    return await i.editReply({ content: question });
  };

  // generator command
  async generator(i: ChatInputCommandInteraction): Promise<Message> {
  ...
  // owo command
  async owo(i: ChatInputCommandInteraction, query: string): Promise<Message> {
    const res = await fetch(`https://nekos.life/api/v2/owoify?text=${query}`).then(res => res.json());
    return await i.editReply({ content: res.owo });
  };
